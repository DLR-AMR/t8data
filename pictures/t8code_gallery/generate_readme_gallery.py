#!/usr/bin/env python3
"""
Regenerate the animated README gallery GIF from the images in this folder.

Every image placed alongside this script is automatically picked up (sorted
by filename) and appended to readme_gallery.gif. A static image becomes one
held frame; an already-animated source has all of its frames spliced in at
their original per-frame durations, so its motion is preserved rather than
collapsed to a single still. Run this script again after adding, removing,
or replacing images in this folder, then copy or upload the resulting GIF to
wherever t8code's README references it.

Usage: python3 generate_readme_gallery.py
"""
...
OUTPUT_PATH = GALLERY_DIR / "readme_gallery.gif"
